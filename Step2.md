...
```
When the import status is completed you can create an instance based on that AMI. However you will get a message like **"...instance type does not support an AMI with TPM version v2.0. Specify an instance type that supports TPM version v2.0, and try again."** if you select an instance type that does not support TPM version v2.0 The lowest priced [instances that I found that support TPM version v2.0](https://docs.aws.amazon.com/AWSEC2/latest/WindowsGuide/enable-nitrotpm-prerequisites.html) are C5 instances.
